...
.. deprecated:: 1.0
   ``pesto`` was formerly known as ``mbanalysis``. Importing ``mbanalysis``
   still works but is deprecated (it now emits a ``FutureWarning``) and
   will be removed in **v1.1**. Use ``from green_mbtools import pesto`` instead.
