...
class pyscf_pbc_init(pyscf_init):
    ...
    def mean_field_input(self, mydf=None):
        ...
        comm.save_data(
            ...
            T,
            hf_dm,
            tools.pbc.madelung(self.cell, self.kmesh),
            Zs,
            last_ao,
        )
    ...
    def compute_df_int(self, nao, X_k):
        ...
        if (
            "gf2" in self.args.finite_size_kind
            or "gw" in self.args.finite_size_kind
            or "gw_s" in self.args.finite_size_kind
        ):
        ...
        #auxcell = gggdf.make_modrho_basis(mydf.cell, mydf.auxbasis, mydf.exp_to_discard)
        #kptij_lst = [(ki, ki) for i, ki in enumerate(self.kmesh)]
        #kptij_lst = np.asarray(kptij_lst)
        #gdf.GDF.weighted_coulG = int_utils.weighted_coulG_ewald
        #logging.info("Generating Ewald corrected 3-center integrals...")
        #gggdf._make_j3c(mydf, self.cell, auxcell, kptij_lst, "cderi_ewald.h5")
        #gdf.GDF.weighted_coulG = None
        #if not new_pyscf:
    ...
    def evaluate_high_symmetry_path(self):
        ...
        inp_data["high_symm_path/r_mesh"] = ft.construct_rmesh(
            self.args.nk, self.args.nk, self.args.nk
        )
    ...
    def compute_twobody_finitesize_correction(self, mydf=None):
        ...
        if "gf2" in self.args.finite_size_kind:
            comm.compute_ewald_correction(
                self.args, self.cell, self.kmesh, self.args.hf_int_path + "/df_ewald.h5"
            )
...
class pyscf_mol_init(pyscf_init):
    ...
    def __init__(self, args=None):
        ...
        self.kmesh = np.array([[0.0, 0.0, 0.0]])
        self.k_ibz = np.array(
            [
                [0.0, 0.0, 0.0],
            ]
    ...
    def compute_df_int(self, nao, X_k):
        ...
        if not isinstance(j3c_obj, h5py.Dataset):  # not a dataset
            if isinstance(j3c_obj, h5py.Group):  # pyscf >= 2.1
                ...
                raise ValueError(
                    "Unknown structure of cderi_mol.h5. Perhaps, PySCF upgrade went badly..."
                )
        else:  # pyscf < 2.1
