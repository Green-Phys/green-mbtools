...
def nevan_run_selfenergy(Sigma_iw, wsample, input_parser, nevan_exe="nevanlinna", outdir="nevanlinna"):
  '''
   Run dim0 times Nevanlinna continuation for Sigma(iw). Note that Sigma(iw) should only lie on positive Matsubara frequency axis.
   :param Sigma_iw: (nw, dim1),
   :param wsample: Matsubara frequency samplings
   :param input_parser[string]: "Sigma_iw_file_to_dump number_of_sampling Sigma_w_file_to_dump coeff" each term is separated by whitespace.
   :param exe_path: Nevanlinna executable path
   :param outdir: output directory w.r.t. the current working directory
   :return:
   '''
  ...
  print("Will dump input to {} and output to {}".format(X_iw_path, X_w_path))
  assert nw == wsample.shape[0], "Number of imaginary frequency points mismatches between \"input_parser\" and wsample."
  assert nw == Sigma_iw.shape[0], "Number of imaginary frequency points mismatches between \"input_parser\" and Gw."
  ...
  for d1 in range(dim1):
    if not os.path.exists(str(d1)):
      os.mkdir(str(d1))
    np.savetxt("{}/{}".format(d1, X_iw_path), np.column_stack((wsample, Sigma_iw[:,d1].real, Sigma_iw[:,d1].imag)))
  ## Start analytical continuation
  processes = []
  pp = 0
  for d1 in range(dim1):
  ...
  if dump_A:
    ...
    for d1 in range(dim1):
      try:
        X_w = np.loadtxt("{}/{}".format(d1, X_w_path))
        if dtype == complex:
          Sigma_w[:,d1].real, Sigma_w[:,d1].imag = X_w[:,1], X_w[:,2]
        else:
          Sigma_w[:,d1] = X_w[:,1]
      except IOError:
        print("{} is missing in {} folder. Possibly analytical continuation fails at that point.".format(X_w_path, d1))
    ...
    print("All AC fails. Will not dump to DOS.h5")
    Sigma_iw = Sigma_iw.reshape(g_shape)
